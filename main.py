...
# Новый список источников (рабочие ссылки из твоего запроса)
# Примечание: Не все являются RSS — для страниц вроде https://hi-tech.mail.ru/tag/neyroseti/ нужен HTML-парсер (добавь BeautifulSoup, если требуется)
RSS_FEEDS = [
    ...
    "https://neuro-ai.ru/news",  # Neuro-AI (HTML-страница, нужен парсер)
    "https://hi-tech.mail.ru/tag/neyroseti/",  # Hi-Tech Mail.ru (тег, HTML)
    "https://vc.ru/tag/%D0%BD%D0%B5%D0%B9%D1%80%D0%BE%D1%81%D0%B5%D1%82%D0%B8",  # VC.ru (тег, HTML)
    "https://www.forbes.ru/tegi/neyroseti",  # Forbes.ru (тег, HTML)
    "https://neurohive.io/ru/novosti/",  # Neurohive (HTML)
    "https://habr.com/ru/hubs/artificial_intelligence/",  # Habr (хаб, HTML)
    "https://aimagazine.com/feed",  # AI Magazine (RSS)
    "https://spectrum.ieee.org/tag/neural-networks/rss",  # IEEE Spectrum (RSS)
    "https://www.technology.org/category/information-processing/ai-neural-networks/feed/",  # Technology.org (RSS)
    "https://www.artificialintelligence-news.com/feed/"  # ArtificialIntelligence-News (RSS)
]
...
def get_one_news():
    ...
    for url in RSS_FEEDS:
        try:
            ...
            r.raise_for_status()
            # Для RSS используем ET; для HTML нужен отдельный парсер (пока пропускаем, если не XML)
            try:
                ...
                print(f"[WARN] {url} не является RSS — нужен HTML-парсер (пропускаем)")
                continue  # Пропускаем не-RSS страницы (добавь BeautifulSoup для полного парсинга)
